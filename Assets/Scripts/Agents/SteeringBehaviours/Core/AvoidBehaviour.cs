...
public class AvoidBehaviour : SteeringBehaviour
{
	...
	public override Vector3 GetAcceleration (MovementStatus status) {
		...
		bool leftHit = Physics.Raycast (transform.position, Quaternion.Euler (0f, -sightAngle, 0f) * status.direction, sightRange);
		bool centerHit = Physics.Raycast (transform.position, status.direction, sightRange);
		bool rightHit = Physics.Raycast (transform.position, Quaternion.Euler (0f, sightAngle, 0f) * status.direction, sightRange);
